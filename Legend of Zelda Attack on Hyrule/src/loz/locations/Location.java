	...
	public void lookForItems(Player player) {
		if (items == null) {
			...
			int[] genItems = {
					...
					(int) (Math.random() * locationInfo.getItemRarity() * 10),
					(int) (Math.random() * 1) };
			if(locationInfo == EnumLocation.SPECIAL_FOREST){
				genItems[3] = 1;	
				player.elements[0] = true;
			}else if(locationInfo == EnumLocation.SPECIAL_MOUNTAIN){
				genItems[3] = 2;	
				player.elements[1] = true;
			}else if(locationInfo == EnumLocation.SPECIAL_LAKE){
				genItems[3] = 3;	
				player.elements[2] = true;
			}else if(locationInfo == EnumLocation.SPECIAL_CASTLE){
				genItems[3] = 4;	
				player.weapon = new Weapon(EnumWeapon.MASTER_SWORD);
			}
		...
		if(locationInfo == EnumLocation.SPECIAL_FOREST){
			GameUtil.println("  The Forest Element");
		}else if(locationInfo == EnumLocation.SPECIAL_MOUNTAIN){
			GameUtil.println("  The Fire Element");
		}else if(locationInfo == EnumLocation.SPECIAL_LAKE){
			GameUtil.println("  The Water Element");
		}else if(locationInfo == EnumLocation.SPECIAL_CASTLE){
			GameUtil.println("  THE MASTER SWORD");
